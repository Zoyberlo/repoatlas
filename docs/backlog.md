...
*Status 2026-09-04, evening:* item 14 is done and went further than
planned. The benchmark it produced was turned on the ranking weights, which
settled every open judgement in `rank/pagerank.py`; see
`docs/benchmarks/weights.md`. Building it required replacing the benchmark's
own metric, which had been rewarding a map that lists filenames and says
nothing.
